...
public class BaseController {
    ...
    @RequestMapping(value="/removecart", params= {"id"}, method=RequestMethod.GET)
    public String removeCart(@RequestParam("id") Integer productId,HttpSession session){
        CartBean cart =(CartBean) session.getAttribute(Constants.ATTR_CART);
        if(cart!=null){
            Product product = this.productService.findByProductId(productId);
            if(product!=null){
                cart.removeProduct(productId);
                session.setAttribute(Constants.ATTR_CART,cart);
